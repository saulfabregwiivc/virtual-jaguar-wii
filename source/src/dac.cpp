...
#define BUFFER_SIZE		0x8000						// Make the DAC buffers 64K x 16 bits
...
void DACInit(void)
{
	...
	desired.freq = 11025;
	desired.format = AUDIO_S16MSB;					// This uses the native endian (for portability)...
	desired.channels = 2;
...
//Of course a better way would be to query the hardware to find the upper limit...
			if (data > 7)	// Anything less than 8 is too high!
			{
				...
				//desired.freq = GetCalculatedFrequency();// SDL will do conversion on the fly, if it can't get the exact rate. Nice!
//desired.freq = 32000;
desired.freq = GetCalculatedFrequency();
